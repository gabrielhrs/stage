...
int main(int argc, char *argv[]) {
	...
	double w_min = atof(argv[5]);

	double w = std::numeric_limits<double>::infinity();
	double t;
	double s2;

	double v_test = atof(argv[6]);

	accumulator_set<double, features<tag::min, tag::max, tag::mean, tag::variance, tag::median, tag::sum> > acc_b;

	std::multiset<double> classes;

	while(w > w_min) {	
		if(!child) {
			for(g_counter = 0; (g_counter < BATCHSIZE) && w > w_min; g_counter++) { // generates a group of children processes 	
				pipe(fd[k%BATCHSIZE]);
		...
		s2 = ((k)/(k-1))*variance(acc_b); 		// unbiased estimator
		boost::math::students_t dist(k-1);
		t = t = boost::math::quantile(complement(dist,conf_level/2));
		w = (t*s2)/sqrt(k);
		std::cout << "CI = [" << mean(acc_b)-w << "," << mean(acc_b)+w << "]" << endl;	
