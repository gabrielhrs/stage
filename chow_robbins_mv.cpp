...
bool cAndRTest(boost::unordered_map<std::string, double> map_d_est, double d) {
	bool result = true;
	BOOST_FOREACH(map_b::value_type i, map_d_est) {
		result = result && i.second <= pow(d,2);
	}
	return result;
}
...
int main(int argc, char *argv[]) {
	...
	while(!(cAndRTest(map_d_est,d)) || keys.empty()) {	
		if(!child) {
			for(g_counter = 0; (g_counter < BATCHSIZE) && (!(cAndRTest(map_d_est,d)) || keys.empty()); g_counter++) { // generates a group of children processes 	
				pipe(fd[k%BATCHSIZE]);
		...
		for(l = 0; l < keys.size(); l++) {
			map_d_est[keys[l]] = ((variance(map_accs.at(keys[l]))+(double)pow(static_cast<double>(k),-1))*(double)pow(static_cast<double>(a),2))/k;
		}		
		std::cout << "\r" << k << flush;
		g_index++;
		}	
	...
	if(!child) {
		...
		for(l = 0; l < keys.size(); l++) {
			...
			std::cout << keys[l] << ": " << "[" << mean(map_accs.at(keys[l])) - sqrt(map_d_est.at(keys[l])) << ","  
												<< mean(map_accs.at(keys[l])) + sqrt(map_d_est.at(keys[l])) << "]" << endl;
		}
