...
static int max_simulation_time = 20000;//10000;
...
extern inline int check_connected_components(int ids[], int best_succ[]);
extern inline void print_net(int ids[], int best_succ[]);
extern inline int build_graph(int ids[], int dests[][24]);

static int alive_table[1000];
static int joined_table[1000];
static s_node_t all_nodes[1000];
static int total_number_of_nodes;

static void fail (node_t node);

static int number_of_failed_lookups;		
static double total_time_lookups;
static int number_of_sucessful_lookups;
...
static int random_lookup(node_t);
...
int node(int argc, char *argv[])
{
  ...
  if (argc == 3) { // first ring
	...
	/***************/
	deadline = DBL_MAX; //poisson_interval_generator(10000);
	/***************/
  ...
  if (join_success) {
	...
	if(0) { 
	    ...
	    next_stabilize_date = init_time + periodic_stabilize_delay;
	    next_fix_fingers_date = init_time + periodic_fix_fingers_delay;
	    next_check_predecessor_date = init_time + periodic_check_predecessor_delay;
	    next_lookup_date = init_time + periodic_lookup_delay;
	 	node.next_finger_to_fix = 0;
	...
	/***************/  
	alive_table[node.id-1] = 1;
	joined_table[node.id-1] = 1;
...
//      && MSG_get_clock() < node.last_change_date + 1000
	  ...
	  /***************/  
  	  all_nodes[node.id-1] = node;
	  
	  int sample1 = poisson_interval_generator(20);  // for 1.091, P(X<2) is approx 0.70
	  int sample2 = poisson_interval_generator(20);  // for 1.091, P(X<2) is approx 0.70

	  /*
	  if(sample1 < 5.5 && node.id != 1 && MSG_get_clock() < 20000) {
		printf("Node %d has failed\n", node.id);
		fail(&node);
		if(MSG_get_clock() < max_simulation_time) {
			MSG_process_sleep(max_simulation_time-MSG_get_clock());
		}
		return 0;
	  }
	  */
	  if(sample2 < 10 && node.id !=1 && MSG_get_clock() < 10000) {
  		//printf("Node %d has left\n", node.id);
		leave(&node);
	  	//xbt_free(node.fingers);
	    int known_id = atoi(argv[2]);
		while(MSG_get_clock() < max_simulation_time && MSG_get_clock() < 10000) {
	  	  int sample3 = poisson_interval_generator(20);
			if (sample3 < 15) {
				if(join_success = join(&node, known_id)) {
					//printf("Node %d has rejoined\n", node.id);
					goto restart;
				}
			}
			MSG_process_sleep(poisson_interval_generator(10));
		}
		return 0;	
	  }	  
	  /***************/  
  ...
  //printf("Node %d\n", node.id);
  //printf("My predecessor is %d\n", node.pred_id);
  //printf("My successor is %d\n", (node.fingers)[0]);
  //printf("Number of attempts %d\n", number_of_attempts);

  for (i = 0; i < nb_bits; i++) {
    //printf("%d ", node.fingers[i].id);
  }
  ...
  // stop the simulation
  //xbt_free(node.fingers);  /***/ modified - commented /***/
  return 0;
}
...
static int join(node_t node, int known_id)
{
  ...
  */
  ...
  if (successor_id == -1) {
	...
	//printf("Node %d told %d that the sucessor is %d\n", known_id, node->id, successor_id);
...
static int find_successor(node_t node, int id)
{
  double t0 = MSG_get_clock();
  // is my successor the successor?
  if (is_in_interval(id, node->id + 1, node->fingers[0].id)) {
	int successor = node->fingers[0].id;
	total_time_lookups += MSG_get_clock() - t0;
	number_of_sucessful_lookups++;
    return successor;
  }

  // otherwise, ask the closest preceding finger in my table
  int closest = closest_preceding_node(node, id);
  int successor = remote_find_successor(node, closest, id);
  if (successor != -1) {	   
  	total_time_lookups += MSG_get_clock() - t0;
	number_of_sucessful_lookups++;
  }
...
static int remote_find_successor(node_t node, int ask_to, int id)
{
  ...
  if (res != MSG_OK) {
    XBT_DEBUG("Failed to send the 'Find Successor' request (task %p) to %d for id %d",
        task_sent, ask_to, id);
	//printf("Failed to send the 'Find Successor' request (task %p) to %d for id %d\n", task_sent, ask_to, id);
    task_free(task_sent);
  }
  else {
 ...
 * \param node the current node
 */
static int random_lookup(node_t node)
{
  int random_index;
  while(1) {
  	random_index = RngStream_RandInt(node->stream, 0, nb_bits - 1);
  	if(alive_table[random_index] && random_index != node->id)
		break;
  }
  int random_id = node->fingers[random_index].id;
  XBT_DEBUG("Making a lookup request for id %d", random_id);
  //printf("Making a lookup request for id %d\n", random_id);
...
/*
static int connectivity_lookup(node_t node) 
{
    return find_successor(node, 0);	
}
*/
...
int create_process()
{
    ...
    /***************/ // Creating extra process
	
    msg_host_t host = MSG_host_self();	
    int argc = 5;
    char** params = xbt_malloc0(sizeof(char *) * argc);
    params[0] = xbt_strdup("");
    params[1] = xbt_strdup("57");
    params[2] = xbt_strdup("1");
    params[3] = xbt_strdup("100");
    params[4] = xbt_strdup("600");
  
    MSG_process_create_with_arguments("node", node, NULL, host, argc, params);

    char** params1 = xbt_malloc0(sizeof(char *) * argc);
    params1[0] = xbt_strdup("");
    params1[1] = xbt_strdup("59");
    params1[2] = xbt_strdup("1");
    params1[3] = xbt_strdup("100");
    params1[4] = xbt_strdup("600");
  
    MSG_process_create_with_arguments("node", node, NULL, host, argc, params1);
...
void count_connected_components() 
{
    ...
    int node_ids[total_number_of_nodes+1];
    int best_successors_ids[total_number_of_nodes+1];
    ...
    for(i = 0; i < 1000; i++) {
  	  if(joined_table[i]) {
  		  ...
  		  if(alive_table[i]) {
  			  ...
  			  //printf("Its successor is %d\n", (all_nodes[i]).fingers[0]);
  			  for(j = 0; j < nb_bits; j++) {
  				  int successor = all_nodes[i].fingers[j].id;
  				  if(alive_table[successor-1] && all_nodes[i].id != successor && !found_best) {
  					  best_successors_ids[k] = successor;
  					  //printf("Its best sucessor is %d\n", successor);
  				  	  found_best = 1;
  				  }
  			  }
  			  if(found_best == 0)
  				  best_successors_ids[k] = all_nodes[i].id;
  			  k++;
  		  }
  		  else {
    ...
    node_ids[total_number_of_nodes] = -1;
    best_successors_ids[total_number_of_nodes] = -1;		
	
    printf("Number of strong connected components %d\n", check_connected_components(node_ids, best_successors_ids));
}
...
{
  xbt_log_control_set("SA.thresh:critical");	
  ...
  MSG_function_register("create_process", create_process);
  ...
  int all_successors_ids[total_number_of_nodes][nb_bits];
	  ...
	  if(joined_table[i]) {
		  ...
		  if(alive_table[i]) {
			  ...
			  for(j = 0; j < nb_bits; j++) {
				  ...
				  if(alive_table[successor-1])
				  	all_successors_ids[k][j] = successor; 	
				  else
					all_successors_ids[k][j] = all_nodes[i].id; 	
			  }
			  if(found_best == 0)
				  best_successors_ids[k] = all_nodes[i].id;
			  k++;
  ...
  //printf("Best Successors\n");

  for(i = 0; i < k+1; i++) {
	  //printf("%d ", best_successors_ids[i]);
  }

  //printf("\n"); 
   
  //printf("Number of failed lookups %d\n", number_of_failed_lookups); 
  
  //printf("Total number of lookups %d\n", number_of_failed_lookups + number_of_sucessful_lookups); 
   
  //printf("Average lookup time %g\n", (double) total_time_lookups/number_of_sucessful_lookups); 
   
  //printf("Number of strong connected components %d\n", check_connected_components(node_ids, best_successors_ids));

  //print_net(node_ids, best_successors_ids);

  //build_graph(node_ids, all_successors_ids);

  //count_connected_components(); 

  char buffer[1024];
  snprintf(buffer, sizeof(buffer), "%s,%d", "or", build_graph(node_ids, all_successors_ids) > 1 ? 1 : 0);
