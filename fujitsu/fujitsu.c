...
int submit(int argc, char *argv[]) {
  ...
  int submit_number = atoi(argv[1]);
  /*
  while(1) {
    MSG_process_sleep(poisson_interval_generator(lambda));
    if(cs[submit_number] && cr[submit_number] < max_q) {
...
/** Sender server function  */
int sender_server(int argc, char *argv[]) {
  ...
  while(1) {
    if(cs[number_sender] < max_q) {
      cs[number_sender]++;
      res = MSG_task_receive(&(task_r), mailbox_recv); // receive message
    }
    if(cs[number_sender] > 0 && gt[number_sender] > 0) {
      //wait(mu_s);
      cs[number_sender]--;
      gt[number_sender] = 0;
    }
...
/** Receiver server function  */
int receiver_server(int argc, char *argv[]) {
