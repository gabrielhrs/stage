...
int main(int argc, char *argv[]) {
  ...
  printf("Total simulation time: %g\n", MSG_get_clock());

  unsigned long l[6] = {1L, 2L, 3L, 4L, 5L, 7L};
  
  //RngStream_SetPackageSeed(l);
  
  RngStream rstream;

  rstream = RngStream_CreateStream("mystream");

  RngStream_SetSeed(rstream, l);
  RngStream_ResetStartStream(rstream);
