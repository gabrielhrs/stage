...
int main(int argc, char *argv[]) {
	...
	while(w > w_min) {	
		...
		s2 = ((k)/(k-1))*variance(acc); 		// unbiased estimator
		boost::math::students_t dist(k-1);
		t = boost::math::quantile(complement(dist,conf_level/2));
		//t = boost::math::quantile(dist,conf_level/2);
		w = (t*s2)/sqrt(k);
		//std::cout << "CI = [" << mean(acc)-w << "," << mean(acc)+w << "]" << endl;	
		std::cout << "\r" << k << flush;
		g_index++;
		}	
	}
